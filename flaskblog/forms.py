...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        ...
        if user:
            raise ValidationError(f'Username "{user.username}" is taken. Log in, or choose another.')
    def validate_email(self, email):
        user = User.query.filter_by(email=email.data).first()
        if user:
            raise ValidationError(f'Email "{user.email}" is taken. Log in, or use another.')
